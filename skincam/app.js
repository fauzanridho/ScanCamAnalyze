...
app.get('/produkimg/pagi/:subfolder/:imageName', async (req, res) => {
  const { subfolder, imageName } = req.params; // Ambil subfolder dan imageName dari params
  const filePath = `produkimg/pagi/${subfolder}/${imageName}`; // Path lengkap ke file

  try {
    // Referensi file di dalam bucket
    ...
    // Generate signed URL untuk file tersebut
    const [url] = await file.getSignedUrl({
      action: 'read',
      expires: '03-09-2491', // Tanggal kedaluwarsa
    });

    // Kirim respons dengan URL
    res.status(200).json({ 
      type: imageName.split('.').pop().toLowerCase(), 
      name: imageName, 
      url 
    });
...
app.get('/produkimg/malam/:subfolder/:imageName', async (req, res) => {
  const { subfolder, imageName } = req.params; // Ambil subfolder dan imageName dari params
  const filePath = `produkimg/malam/${subfolder}/${imageName}`; // Path lengkap ke file
